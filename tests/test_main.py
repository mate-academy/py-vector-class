...
def test_vector_instance():
    vector = Vector(3, 4)
    assert hasattr(vector, 'x_coordinate'), (
        "Vector instance should have attribute 'x_coordinate'"
    )
    assert hasattr(vector, 'y_coordinate'), (
        "Vector instance should have attribute 'y_coordinate'"
    )


def test_vector_decimals():
    vector = Vector(-2.343, 8.008)
    assert (vector.x_coordinate, vector.y_coordinate) == (-2.34, 8.01), (
        "Attributes 'x_coordinate', 'y_coordinate' should be rounded to two decimals."
    )
...
def test_vector_add(point1_x, point1_y, point2_x, point2_y, point3_x, point3_y):
    ...
    assert (vector3.x_coordinate, vector3.y_coordinate) == (point3_x, point3_y), (
...
def test_vector_mul_number(vector1_x, vector1_y, number, vector2_x, vector2_y):
    ...
    assert (vector2.x_coordinate, vector2.y_coordinate) == (vector2_x, vector2_y), (
...
def test_create_vector_by_two_points(start_point, end_point, vector_coords):
    ...
    assert (vector.x_coordinate, vector.y_coordinate) == vector_coords, (
...
def test_get_normalized(coords, normalized_coords):
    ...
    assert (normalized_vector.x_coordinate, normalized_vector.y_coordinate) == normalized_coords, (
        f"When 'vector' coords equals to {coords}, "
...
def test_rotate(coords_1, degrees, coords_2):
    ...
    assert (vector2.x_coordinate, vector2.y_coordinate) == coords_2, (
        f"When 'vector' coordinates equal to {coords_1}, "
