...
def test_vector_instance():
    vector = Vector(3, 4)
    assert hasattr(vector, 'x_point'), (
        "Vector instance should have attribute 'x_point'"
    )
    assert hasattr(vector, 'y_point'), (
        "Vector instance should have attribute 'y_point'"
    )


def test_vector_decimals():
    vector = Vector(-2.343, 8.008)
    assert (vector.x_point, vector.y_point) == (-2.34, 8.01), (
        "Attributes 'x_point', 'y_point' should be rounded to two decimals."
    )
...
def test_vector_add(point1_x, point1_y, point2_x, point2_y, point3_x, point3_y):
    ...
    assert (vector3.x_point, vector3.y_point) == (point3_x, point3_y), (
...
def test_vector_mul_number(vector1_x, vector1_y, number, vector2_x, vector2_y):
    ...
    assert (vector2.x_point, vector2.y_point) == (vector2_x, vector2_y), (
...
def test_create_vector_by_two_points(start_point, end_point, vector_coords):
    ...
    assert (vector.x_point, vector.y_point) == vector_coords, (
...
def test_get_normalized(coords, normalized_coords):
    ...
    assert (normalized_vector.x_point, normalized_vector.y_point) == normalized_coords, (
        f"When 'vector' coords equals to {coords}, "
...
def test_rotate(coords_1, degrees, coords_2):
    ...
    assert (vector2.x_point, vector2.y_point) == coords_2, (
        f"When 'vector' coordinates equal to {coords_1}, "
