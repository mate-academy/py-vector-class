...
def test_vector_instance():
    vector = Vector(3, 4)
    assert hasattr(vector, 'vector_x'), (
        "Vector instance should have attribute 'vector_x'"
    )
    assert hasattr(vector, 'vector_y'), (
        "Vector instance should have attribute 'vector_y'"
    )


def test_vector_decimals():
    vector = Vector(-2.343, 8.008)
    assert (vector.vector_x, vector.vector_y) == (-2.34, 8.01), (
        "Attributes 'vector_x', 'vector_y' should be rounded to two decimals."
    )
...
def test_vector_add(point1_x, point1_y, point2_x, point2_y, point3_x, point3_y):
    ...
    assert (vector3.vector_x, vector3.vector_y) == (point3_x, point3_y), (
...
def test_vector_mul_number(vector1_x, vector1_y, number, vector2_x, vector2_y):
    ...
    assert (vector2.vector_x, vector2.vector_y) == (vector2_x, vector2_y), (
...
def test_create_vector_by_two_points(start_point, end_point, vector_coords):
    ...
    assert (vector.vector_x, vector.vector_y) == vector_coords, (
...
def test_get_normalized(coords, normalized_coords):
    ...
    assert (normalized_vector.vector_x, normalized_vector.vector_y) == normalized_coords, (
        f"When 'vector' coords equals to {coords}, "
...
def test_rotate(coords_1, degrees, coords_2):
    ...
    assert (vector2.vector_x, vector2.vector_y) == coords_2, (
        f"When 'vector' coordinates equal to {coords_1}, "
