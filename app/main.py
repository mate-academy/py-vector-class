...
class Vector:
    ...
    def get_angle(self) -> int:
        return self.angle_between(Vector(float(0), math.fabs(self.y)))

    def rotate(self, degrees: int) -> Vector:
        return Vector(
            self.x * math.cos(math.radians(degrees)) - self.y * math.sin(
                math.radians(degrees)
            ),
            self.x * math.sin(math.radians(degrees)) + self.y * math.cos(
                math.radians(degrees)
            )
        )
