...
class Vector:
    ...
    def angle_between(self, other: Vector) -> int:
        return round(math.degrees(math.acos((self.x * other.x + self.y * other.y) / (((self.x ** 2 + self.y ** 2) ** 0.5) * (other.x ** 2 + other.y ** 2) ** 0.5))))

    def get_angle(self) -> int:
        if self.x < 0 and self.y < 0:
            return round(180 - (math.degrees(math.acos((self.x * 0 + self.y * self.y) / (
                        ((self.x ** 2 + self.y ** 2) ** 0.5) * ((self.x ** 2 + self.y ** 2) - self.x ** 2) ** 0.5)))))
        return round(math.degrees(math.acos((self.x * 0 + self.y * self.y) / (((self.x ** 2 + self.y ** 2) ** 0.5) * ((self.x ** 2 + self.y ** 2) - self.x ** 2) ** 0.5))))
