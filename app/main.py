...
class Vector:
    ...
    def dot_product(vec_1: Vector, vec_2: Vector) -> float:
        return vec_1.coord_x * vec_2.coord_x + vec_1.coord_y * vec_2.coord_y
    ...
    def def_len(vec: Vector) -> float:
        return (vec.coord_x ** 2 + vec.coord_y ** 2) ** 0.5
    ...
    def get_normalized(self) -> Vector:
        ...
        return Vector(self.coord_x / len_, self.coord_y / len_)
    ...
    def get_angle(self) -> int:
        other = Vector(0, int(math.fabs(self.coord_y)))
        return self.def_angle(self, other)

    def rotate(self, angel: int) -> Vector:
        ang = math.radians(angel)
        return Vector(
            self.coord_x * math.cos(ang) - self.coord_y * math.sin(ang),
            self.coord_x * math.sin(ang) + self.coord_y * math.cos(ang)
        )
