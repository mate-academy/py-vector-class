...
class Vector:
    ...
    def __mul__(self, other: Vector) -> Vector:
        if isinstance(other, (int, float)):
            return Vector(self.x_cord * other, self.y_cord_cord * other)
    ...
    def get_normalized(self) -> Vector:
        return Vector((self.x_cord / self.get_length()), (self.y_cord / self.get_length()))
    ...
    def rotate(self, degr: int | float) -> Vector:
        theta = radians(degr)
        x = self.x_cord * cos(theta) - self.y_cord * sin(theta)
        y = self.x_cord * sin(theta) + self.y_cord * cos(theta)
